...
- Base content: `z-0`
- In-surface sticky elements (card/page header/footer): `z-10`
- Floating UI (dropdown/popover/tooltip/menu): `z-30`
- Backdrops / overlays: `z-40`
- Modal / drawer / dialog surfaces: `z-50`
- Toast / global notifications: `z-60`
...
- InputGroup
  - Props: `children`, `disabled?`, `loading?`, `invalid?`, `orientation? enum: horizontal|vertical`, `attached?`
  - StyleProps: `size? enum: sm|md|lg`, `variant? enum: outline|filled|ghost`, `className?`
  - Sub-components: children (TextInput / Button / Select etc.)
...
- RadioGroup
  - Props: `items RadioGroupItem[]`, `value?`, `defaultValue?`, `onChange?`, `name?`, `required?`, `disabled?`, `orientation? enum: horizontal|vertical`, `errorMessage?`, `children?`
  - StyleProps: `size? enum: sm|md|lg`, `className?`
...
- FieldLayout(internal) — presentational stack: optional label → control slot → optional error (spacing only; control chrome stays on the input). Defined in `fieldPieces` with `fieldSurfaceVariants` for custom text-like controls.
...
- Toggle
  - Props: `pressed?`, `defaultPressed?`, `onChange?`, `disabled?`, `left?`, `right?`, `ariaLabel?`, `loading?`, `children?`
  - StyleProps: `size? enum: sm|md|lg`, `variant? enum: primary|secondary|outline|ghost`, `className?`
  - Sub-components: Button

- ToggleGroup
  - Props: `items ToggleGroupItem[]`, `value?`, `defaultValue?`, `onChange?`, `multiple?`, `orientation? enum: horizontal|vertical`, `attached?`, `disabled?`, `children?`
  - StyleProps: `size? enum: sm|md|lg`, `variant? enum: primary|secondary|outline|ghost`, `className?`
  - `ToggleGroupItem: { label, value, left?, disabled? }`
...
- CopyButton
  - Props: `value`, `onCopy?`, `label?`, `copiedLabel?`, `timeout?`, `left?`, `tooltip?`, `disabled?`, `loading?`, `children?`
  - StyleProps: `variant? enum: primary|secondary|outline|ghost`, `size? enum: sm|md|lg`, `className?`
  - SlotProps: `buttonProps?`, `tooltipProps?`
...
- Link
  - Props: `href`, `target? enum: _self|_blank`, `rel?`, `external?`, `children?`
  - StyleProps: `variant? enum: default|muted|underline`, `className?`
  - Note: Auto-applies `rel="noopener noreferrer"` when `external` or `target="_blank"`. Replaces bare `<a>` usage. (uses `fieldPieces` focus-ring tokens; compose with `Text` as children when needed)

- Pill — unified chip (counts, status, filters, removable tags). **`Badge`** and **`Tag`** remain export aliases for compatibility (`Tag` maps `label` + surface `variant` → `appearance`).
  - Props: `as? enum: span|button`, `left?`, `right?`, `onRemove?`, `dot?`, `tone? enum: neutral|info|success|warning|danger`, `variant? enum: default|muted|subtle|code|danger|outline` (Typography / `Text`; not the same union as `appearance`), `disabled?`, `children?`
  - StyleProps: `appearance? enum: solid|subtle|outline|ghost`, `size? enum: sm|md`, `shape? enum: rounded|pill`, `className?`
  - Sub-components: Text
...
- PillGroup
  - Props: `items PillItem[]`, `value?`, `defaultValue?`, `onChange?`, `selectable?`, `onSelect?`, `multiple?`, `removable?`, `onRemove?`, `maxVisible?`, `overflowLabel?`, `children?`
  - StyleProps: `size? enum: sm|md`, `variant? enum: solid|subtle|outline|ghost` (Pill `appearance`), `gap? enum: sm|md|lg`, `wrap?`, `className?`
  - `PillItem: { label, value, tone? enum: neutral|info|success|warning|danger, disabled?, left?, selected? }`
  - Note: Wrap-friendly pill selector/display group. For dropdown-based multi-select, use `Select` with `multiple`.
  - Sub-components: Pill
...
- Image
  - Props: `src`, `alt?`, `loading?`, `loadingStrategy? enum: lazy|eager`, `fallback?`, `placeholder? enum: blur|skeleton|none`
...
- Alert (inline contextual message)
  - Props: `title?`, `description?`, `left?`, `action?`, `tone? enum: neutral|info|success|warning|danger`, `dismissible?`, `onClose?`, `children?`
  - StyleProps: `variant? enum: solid|subtle|outline`, `className?`
  - Note: Replaces `Banner`. For page-level notification bars use `variant="solid"` with full-width layout.
...
- Overlay (backdrop primitive)
  - Props: `open?`, `onClose?`, `container?`, `children?`
  - StyleProps: `blur?`, `className?`
...
- PageHeader (in-page section header — not the `<header>` element)
...
- EmptyState
  - Props: `title?`, `description?`, `left?`, `action?`
  - StyleProps: `variant? enum: default|minimal|spacious|error`, `size? enum: sm|md|lg`, `className?`
  - `action: { label, onClick, loading?, disabled? }`
  - Sub-components: Text, Button, Icon
